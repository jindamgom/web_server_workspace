        ...
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form name="memberUpdateFrm" action="${pageContext.request.contextPath}/member/memberUpdate" method="post" class="space-y-4 md:space-y-6" >
                <div>
                    ...
                    <input type="text" name="id" id="id" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" value="${sessionScope.loginMember.id}" readonly required>
                </div>
                <div>
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 after:content-['*'] after:ml-0.5 after:text-red-500">이름</label>
                    <input type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" value="${sessionScope.loginMember.name}" required>
                </div>
                <div>
                    <label for="birthday" class="block mb-2 text-sm font-medium text-gray-900">생일</label>
                    <input type="date" name="birthday" id="birthday" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" value="${sessionScope.loginMember.birthday}" >
                </div>
                <div>
                    ...
                    <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="name@company.com" value="${sessionScope.loginMember.email}" required>
                </div>
                <div>
                    <label for="phone" class="block mb-2 text-sm font-medium text-gray-900">전화번호</label>
                    <input type="text" name="phone" id="phone" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" placeholder="(-) 없이 입력하세요." value="${sessionScope.loginMember.phone}">
                </div>
                <fieldset>
                    ...
                    <div class="inline-flex items-center mr-4">
                        ...
                        ${sessionScope.loginMember.gender=='M'?'checked':''}>
                    ...
                    <div class="inline-flex items-center mr-4">
                        ...
                        ${sessionScope.loginMember.gender== Gender.F ?'checked':''}>
                ...
                <fieldset>
                    ...
                    <%-- core의 set Tag : context 객체별 속성 생성(c:remove와 대조됨) --%>
...
<%--                        <c:if test="${hobby!='운동' && hobby!='게임'&& hobby!='영화' && hobby!='음악'}">--%>
                        <c:if test="${not hobbyDefaultList.contains(hobby)}">
                            <div class="inline-flex items-center mr-4">
                                <input id= "hobby-${vs.count+hobbyDefaultList.size()}" type="checkbox" name="hobby" value="${hobby}" checked class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2" ${loginMember.hobby.contains('음악')? 'checked' : ''}>
                ...
                <div>
                    ...
                    <input type="number" name="point" id="point" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"  value="${sessionScope.loginMember.point}" readonly>
                </div>
